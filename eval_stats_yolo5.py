...
def write_to_csv(threshold, video_id, frame_name, object_counts, acc, binary_decision, binary_acc, iou_results):
    csv_file = f"results/{threshold}/{video_id}.csv"
...
def calculate_and_write_averages(threshold, video_id):
    csv_file = f"results/{threshold}/{video_id}.csv"
    ...
    iou_data = data['IoU Results'].apply(eval)
    total_detected_objects = sum(len(iou) for iou in iou_data)
    total_iou_sum = sum(sum(iou) for iou in iou_data)
    avg_iou = total_iou_sum / total_detected_objects if total_detected_objects > 0 else np.nan
...
if __name__ == "__main__":
    image_root_path = "results/keyframes/"
    ...
    # This needs to correspond to the initial value and interval in step1
    start_threshold = 0.4
    end_threshold = 0.6
    interval = 0.05
    ...
    for threshold in ssim_thresholds:
        threshold_folder = os.path.join(image_root_path, str(threshold))
        paths = get_all_image_paths(threshold_folder)
        ...
        for video_id in video_ids:
            ...
            for current_frame_path in video_paths:
                ...
                write_to_csv(str(threshold), video_id, frame_name, object_counts, acc, binary_decision, binary_acc, iou_results)

            # Write the average values to the last row of the CSV file
            calculate_and_write_averages(str(threshold), video_id)
